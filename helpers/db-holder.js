...
const HelperPasaje = {
    existepasajeid: async (id, req) => {
        const existe = await InformacionPasaje.findById(id)

        if (!existe) {
            throw new Error(`El id de pasaje no existe ${id}`)
        }

        req.req.informacionpasajeUpdate = existe
